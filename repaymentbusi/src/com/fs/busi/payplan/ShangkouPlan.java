...
//        String hkri="20";//每期20号还款
        ...
        for (int i=0;i<=qiciSize;++i){
            ...
            if (i == 0) {//第零期开始计息
                //生成日期
                dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qiciShouQi);
                ...
                yhbj = RepayTool.calBenjin(benjinEntity,dateParam);
                //计算费用

                int fwfFs=feeFsMap.get(FeeEnum.fwfFee);

                fwfFee = RepayTool.calFee(fkje, fwflv, jixiDays, fwfFs, true);

                int qdfFs= feeFsMap.get(FeeEnum.qdfFee);
                qudfFee = RepayTool.calFee(fkje, qudfflv, jixiDays, qdfFs, true);
                fee = fwfFee + qudfFee;

            }else if (i==qiciSize-1) {//倒数第二期，一般用于末期俩还款日拆分
                if (kouxiFs==JihuaParam.kouxiFs2&&jixiFs!=JihuaParam.jixiFs21) {
                    dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qiciMoQi);
                    ...
                    dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qiciZhongQi);
                    ksrq=dateParam.ksrq;//开始日期等于上次结束日期
                    ...
                    int qdfFs= feeFsMap.get(FeeEnum.qdfFee);
                    if(fwfFs==JihuaParam.feeFs2)
                        fwfFee = RepayTool.calFee(fkje, fwflv, lixiMonth, fwfFs, true);
                    if(qdfFs==JihuaParam.feeFs2)
                        qudfFee = RepayTool.calFee(fkje, qudfflv, lixiMonth, qdfFs, true);
                    fee = fwfFee + qudfFee;
                }
                ...
                if(kouxiFs==JihuaParam.kouxiFs2&&jixiFs!=JihuaParam.jixiFs21){
                    dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qicijiBen);
                ...
                if(kouxiFs==JihuaParam.kouxiFs2&&yxnbJixi==JihuaParam.jixiFs4){
                    if(dateParam.jsrq.compareTo(year1)>=0&&dateParam.ksrq.compareTo(year1)<0){
                        ...
                        dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qicijiBen);
                        budgetEntity= RepayTool.budget(budgetEntity,JihuaParam.qicijiBen, dateParam, lixiEntity, benjinEntity, feeEntity, 0);
                        ...
                        dateParam= RepayTool.buildRiqi(dateParam, JihuaParam.qiciZhongQi);
                        budgetEntity= RepayTool.budget(budgetEntity,JihuaParam.qiciZhongQi, dateParam, lixiEntity, benjinEntity, feeEntity, lixiMonth);
                        hkjihuaObjList=RepayTool.genQici( hkjihuaObjList,dateParam, budgetEntity, orderNo, i);
                        continue;
